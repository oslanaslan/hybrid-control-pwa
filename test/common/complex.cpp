...

    {
        ...
        for (std::size_t i = 0; i < polygons.size(); i++) {
            for (std::size_t mask_index = 0; mask_index < masks.size();
                 mask_index++) {
                if (((~polygons[i].is_gt) & masks[mask_index])
                    == masks[mask_index]) {
...
TEST(complex, symbolic3d) {
...

    {
        ...
        constexpr int kDim
            = hcpwa::VectorSize<decltype(resolutions.front().second)>() - 1;
        auto [lines, masks] = hcpwa::ResolutionsToMasks<kDim>(resolutions);
        auto polygons = hcpwa::SplitAABBWithLines(
            {aabb.first[0, 1], aabb.second[0, 1]}, lines);
...
TEST(complex, intersect) {
    ...
    auto polygon57 = hcpwa::SplitAABBWithLines(
        aabb2d, hcpwa::DimensionCast<2, 8>(lines57, {5 - 1, 7 - 1}));
    auto polygon84 = hcpwa::SplitAABBWithLines(
        aabb2d, hcpwa::DimensionCast<2, 8>(lines84, {4 - 1, 8 - 1}));
    auto polygon86 = hcpwa::SplitAABBWithLines(
    ...
    std::vector<hcpwa::TriangleWithUniqueVertices> triangles51
        = hcpwa::GetTrianglesWithUniqueVertices(aabb2d, polygon51);
    std::vector<hcpwa::TriangleWithUniqueVertices> triangles57
        = hcpwa::GetTrianglesWithUniqueVertices(aabb2d, polygon57);
    std::vector<hcpwa::TriangleWithUniqueVertices> triangles84
        = hcpwa::GetTrianglesWithUniqueVertices(aabb2d, polygon84);
    std::vector<hcpwa::TriangleWithUniqueVertices> triangles86
        = hcpwa::GetTrianglesWithUniqueVertices(aabb2d, polygon86);
    ...
    GTEST_COUT << "Prisms count: " << prisms51.size() << " " << prisms57.size()
               << " " << prisms84.size() << " " << prisms86.size() << "\n";
