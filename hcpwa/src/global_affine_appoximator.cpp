...
double GlobalAffineApproximator::getBetaParamForAxis(int i, int j) const {
    if (i == 5 - 1 && j == 1 - 1) {
        ...
        throw std::invalid_argument(
            std::string("No such beta params for axis (") + std::to_string(i)
            + ", " + std::to_string(j) + ")");
    }
}
...
void GlobalAffineApproximator::buildLPSegmentForJ(
    ...
    if (vertices_j.empty()) {
        throw std::runtime_error(
            "build_lp_segment_for_j: vertices_j is empty.");
    }
...
GlobalAffineApproximator::prepareLpMatrices(int phase) {
    ...
    for (int j = 0; j < n_areas; ++j) {
        buildLPSegmentForJ(a_j_matrs[j], f_j_vecs[j], q_c_j_matrs[j],
                           q_c_j_vecs[j], q_r_j_matrs[j], q_r_j_vecs[j],
                           g_j_vecs[j], g_j_scals[j],
...
double GlobalAffineApproximator::getBorderFuncValuesAtN(
    ...
    try {
        std::vector<double> x = getValueFunction(this->value_function_, phase,
                                                 r, theta_idx, theta_end_idx);
        double result = 0.0;
        for (int i = 0; i < kSpaceDim; ++i) {
            result += x[i] * n(i);
        }
...
std::vector<double> GlobalAffineApproximator::getBorderConditions(
    ...
    for (int i = 0; i < m; ++i) {
        ...
        for (int j = 0; j < n; ++j) {
            const double a_val
                = -a_matr_lst[i][j];        // Negate for constraint format
    ...
    st = highs.addRows(m, row_lower.data(), row_upper.data(), nnz,
...
void GlobalAffineApproximator::updateHighsRhsUpperBounds(
    ...
    auto st = highs_solver->changeRowsBounds(row_upper.size(), row_ids.data(),
                                             row_lower.data(),
                                             new_row_upper.data());
    if (st != HighsStatus::kOk) {
        throw std::runtime_error(
