...
TEST(value_function_utils, setValueFunction_and_getValueFunction) {
    ...
    hcpwa_util::setValueFunction(vf, 0, 1, 2, 3, data, 3);
    std::vector<double> got = hcpwa_util::getValueFunction(vf, 0, 1, 2, 3);
...
TEST(value_function_utils, setValueFunction_throws_on_wrong_size) {
    ...
    EXPECT_THROW(
        hcpwa_util::setValueFunction(vf, 0, 0, 0, 0, data, 3),
...
TEST(value_function_utils, setValueFunction_throws_when_location_already_used) {
    hcpwa_util::ValueFunction vf;
    std::vector<double> data = {1.0, 2.0};
    hcpwa_util::setValueFunction(vf, 0, 0, 0, 0, data, 2);
    EXPECT_THROW(
        hcpwa_util::setValueFunction(vf, 0, 0, 0, 0, std::vector<double>{3.0, 4.0}, 2),
        std::runtime_error);
}

TEST(value_function_utils, getValueFunction_throws_when_not_found) {
    hcpwa_util::ValueFunction vf;
    EXPECT_THROW(hcpwa_util::getValueFunction(vf, 99, 99, 99, 99),
                 std::invalid_argument);
}

TEST(value_function_utils, dumpValueFunctionToJson_writes_valid_structure) {
    hcpwa_util::ValueFunction vf;
    hcpwa_util::setValueFunction(vf, 0, 1, 2, 3, {1.0, 2.0}, 2);
    hcpwa_util::setValueFunction(vf, 0, 1, 2, 4, {3.0, 4.0, 5.0}, 3);
    hcpwa_util::setValueFunction(vf, 1, 0, 0, 0, {0.5}, 1);

    std::filesystem::path tmp = std::filesystem::temp_directory_path() / "value_function_utils_test.json";
    ASSERT_NO_THROW(hcpwa_util::dumpValueFunctionToJson(vf, tmp.string()));
...
TEST(value_function_utils, dumpValueFunctionToJson_throws_on_bad_path) {
    hcpwa_util::ValueFunction vf;
    hcpwa_util::setValueFunction(vf, 0, 0, 0, 0, {1.0}, 1);
    EXPECT_THROW(
        hcpwa_util::dumpValueFunctionToJson(vf, "/nonexistent/dir/out.json"),
        std::runtime_error);
}
